...
function DomainBubbles({items, behind = false}: {items: typeof domains; behind?: boolean}) {
  return (
    <div
      className={cn("pointer-events-none absolute inset-0", behind ? "z-2" : "z-3")}
      role="group"
...
export function HeroArtwork() {
  return (
    <>
      <DomainBubbles behind items={domains.filter((domain) => domain.behind)} />
      ...
      <DomainBubbles items={domains.filter((domain) => !domain.behind)} />
    </>
  )
}
